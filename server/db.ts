...
// One-time schema create (drop this after first successful run)
setTimeout(async () => {
  try {
    const db = await getDb();
    if (!db) return;
    const pool = (db as any).$client;
    if (!pool?.promise) { console.log("[Schema] no pool"); return; }
    ...
    const createStatements = [
      ...
      `CREATE TABLE IF NOT EXISTS subscriptions (
        ...
        aiMonthlyLimit INT NOT NULL DEFAULT 5,
      ...
      `CREATE TABLE IF NOT EXISTS seenTweets (
        id INT AUTO_INCREMENT PRIMARY KEY,
        userId INT NOT NULL,
        ...
        createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX seenTweets_userId_idx (userId),
        INDEX seenTweets_snoozeUntil_idx (snoozeUntil),
        FOREIGN KEY (userId) REFERENCES users(id) ON DELETE CASCADE
      )`,
      `CREATE TABLE IF NOT EXISTS dailyStats (
        ...
        createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX dailyStats_userId_date_idx (userId, date),
        FOREIGN KEY (userId) REFERENCES users(id) ON DELETE CASCADE
      )`,
      `CREATE TABLE IF NOT EXISTS filterRules (
        ...
        createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX filterRules_userId_idx (userId),
        FOREIGN KEY (userId) REFERENCES users(id) ON DELETE CASCADE
      )`,
      `CREATE TABLE IF NOT EXISTS mutedAccounts (
        id INT AUTO_INCREMENT PRIMARY KEY,
        userId INT NOT NULL,
        ...
        createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX mutedAccounts_userId_idx (userId),
        FOREIGN KEY (userId) REFERENCES users(id) ON DELETE CASCADE
      )`,
      ...
      `CREATE TABLE IF NOT EXISTS deviceSessions (
        ...
        createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX deviceSessions_userId_idx (userId),
        FOREIGN KEY (userId) REFERENCES users(id) ON DELETE CASCADE
      )`
    ];
    ...
    for (const s of createStatements) {
      try {
        await p.query(s);
        const name = s.match(/CREATE TABLE IF NOT EXISTS (\w+)/)?.[1];
        console.log("[Schema] OK:", name);
      } catch (e: any) {
        console.log("[Schema] err:", e?.code, e?.sqlMessage?.slice(0, 100));
      }
    }
    console.log("[Schema] All done!");
  } catch (e: any) {
    console.log("[Schema] failed:", e?.message?.slice(0, 120));
  }
}, 5000);
